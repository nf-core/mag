...
workflow BINNING {
    ...
    // Short reads: either BAMs (bowtie2) or raw reads (CoverM native mapper)
    if (params.coverm_mapper == 'bowtie2') {
    ...
    ch_combined_depths = COVERM_CONTIG_LONGREAD.out.coverage.mix(
        COVERM_CONTIG_SHORTREAD.out.coverage
    )
