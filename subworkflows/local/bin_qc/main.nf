...
workflow BIN_QC {
    ...
    if (params.binqc_tool == "busco" || "busco" in binqc_tool_extras) {
        ...
        ch_versions = ch_versions.mix(BUSCO_BUSCO.out)
    ...
    if (params.binqc_tool == "checkm" || "checkm" in binqc_tool_extras) {
        ...
        ch_versions = ch_versions.mix(CHECKM_LINEAGEWF.out)
        ...
        ch_versions = ch_versions.mix(CHECKM_QA.out)
    ...
    if (params.binqc_tool == "checkm2" || "checkm2" in binqc_tool_extras) {
        ...
        ch_versions = ch_versions.mix(CHECKM2_PREDICT.out)
