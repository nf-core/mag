...
workflow ANCIENT_DNA_ASSEMBLY_VALIDATION {
    ...
    PYDAMAGE_ANALYZE(
        input.map { meta, _contigs, bam, bai ->
            [
                meta,
                bam[0],
                bai[0],
    ...
    ch_versions = ch_versions.mix(SUMMARISEPYDAMAGE.out.versions.first())
    ...
    if (!params.skip_ancient_damagecorrection) {
        ...
        BCFTOOLS_CONSENSUS(
            BCFTOOLS_VIEW.out.vcf.join(BCFTOOLS_INDEX_POST.out.tbi).join(input.map { item -> [item[0], item[1]] })
        )
