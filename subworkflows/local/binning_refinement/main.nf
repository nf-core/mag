...
workflow BINNING_REFINEMENT {
    ...
    ch_versions = ch_versions.mix(RENAME_PREDASTOOL.out)


    // Generate DASTool auxilary files
    DASTOOL_FASTATOCONTIG2BIN_METABAT2(ch_bins_for_fastatocontig2bin.metabat2, "fa")
    ch_versions = ch_versions.mix(DASTOOL_FASTATOCONTIG2BIN_METABAT2.out)
    ...
    ch_versions = ch_versions.mix(DASTOOL_FASTATOCONTIG2BIN_MAXBIN2.out)

    DASTOOL_FASTATOCONTIG2BIN_CONCOCT(ch_bins_for_fastatocontig2bin.concoct, "fa")
    ch_versions = ch_versions.mix(DASTOOL_FASTATOCONTIG2BIN_CONCOCT.out)
    ...
    ch_versions = ch_versions.mix(DASTOOL_DASTOOL.out)
    ...
    ch_versions = ch_versions.mix(RENAME_POSTDASTOOL.out)
