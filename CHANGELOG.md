...
- [#NN](https://github.com/nf-core/mag/pull/NN) - Fix `--checkm_download_url` blocking parameter validation for every run (even with `--run_checkm` off) whenever Zenodo isn't reachable, by dropping its unconditional `exists` schema check (by @erikrikarddaniel)
