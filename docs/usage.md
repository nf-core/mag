...
- Add the configuration block:
  ...
  where `database` is the name of the bind-mount target directory that will be created in `$NXF_TASK_WORKDIR`, and `<image_top-level-directory>` is the top-level directory of the image file-system. There is nothing special about the name `database`.  The `<image_top-level-directory>` can be determined with,
