...
The pipeline uses [ALE (Assembly Likelihood Estimator)](https://github.com/sc932/ALE) to perform probabilistic quality assessment of short-read assemblies generated by MEGAHIT, SPAdes, and SPAdesHybrid. ALE evaluates how likely it is that the observed sequencing reads were produced by a given assembly, providing a reference-free measure of assembly quality that is useful for novel or complex metagenomes.nf
...
Note that ALE only works with short-read assemblies (MEGAHIT, SPAdes). Long-read assemblies (Flye, MetaMDBG) are not supported by ALE, and hybrid assemblies (SPAdesHybrid) use only the short-read component for ALE scoring.
